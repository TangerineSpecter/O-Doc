...
下图对应当前代码实现。橙色节点表示可能调用模型或 embedding 服务、从而产生模型侧 Token/调用费用的路径；其余节点只进行数据库、ChromaDB 或飞书 API 操作。

```mermaid
flowchart TD
    A[收到飞书文本消息] --> B{是否为 /new？}
    B -- 是 --> C[切换 conversation_id\n清空当前 summary]
    C --> D[回复“已开启新对话”\n不调用模型、不写短期记忆]
    B -- 否 --> E[读取长期记忆]
    E --> F[为当前问题生成 embedding]
    F --> G[ChromaDB 召回短期记忆\n更新召回次数与相关度]
    G --> H[组装 system prompt、长期/短期记忆、summary、完整历史]
    H --> I{历史是否超出 48k 预算？}
    I -- 是 --> J[调用聊天模型压缩旧历史为 summary]
    I -- 否 --> K[跳过 summary 压缩]
    J --> L[调用聊天模型生成回复]
    K --> L
    L --> M[回复飞书并保存消息记录]
    M --> N[为本轮 user + assistant 生成 embedding]
    N --> O[写入短期记忆\nDB 元数据 + ChromaDB 向量]

    P[每日调度器\n默认 03:00 后首次轮询] --> Q[筛选候选短期记忆\n30 天内、召回≥3、相关度≥0.8、来源≥3]
    Q --> R{是否有候选？}
    R -- 否 --> S[清理已过期短期记忆\n不调用模型]
    R -- 是 --> T[逐条调用聊天模型\n判断是否晋升长期记忆]
    T --> U[标记已处理；符合条件则创建长期记忆]
    U --> S

    classDef billed fill:#fff3e0,stroke:#f59e0b,color:#7c2d12;
    class F,J,L,N,T billed;
```

费用触发结论：

- 正常文本对话会调用聊天模型生成回复；在短期记忆功能可用时，还会在召回和写入阶段各调用一次 embedding 服务。
- 仅当上下文超出预算时，才会额外调用聊天模型压缩 summary。
- 记忆调度器每天都会运行一次筛选和 TTL 清理，但**不会因此无条件调用模型**。只有短期记忆同时满足图中的四项晋升条件时，才会对每条候选记忆额外调用一次聊天模型；模型成功判定“不保存”后也会标记该条记忆已处理，不会在后续每天重复判断。若该次模型调用异常或返回无法解析的结果，则不会标记为已处理，符合条件的同一条记忆会在次日再次尝试。
- 调度器进程启动后每 30 秒检查一次时间，但通过 `last_run_date` 保证当天只执行一次实际晋升/清理任务。默认开关为 `ODOC_ENABLE_AGENT_MEMORY_SCHEDULER=true`；设置为 `false` 可关闭这条每日任务路径。
...
Agent Memory scheduler 默认启用，在服务进程中于每天 03:00 后首次轮询时运行一次；轮询间隔默认 30 秒。它会将当天执行日期写入 `SystemSetting.system_agent_memory_runtime.last_run_date`，避免同一进程在当天重复执行。
...
符合条件后，系统才会让模型对该条记忆进行一次晋升判断。这是定时任务中唯一的模型调用，也是每日可能新增 Token 消耗的来源。只有稳定偏好、长期事实、长期项目和明确指令才会晋升；临时任务、寒暄和一次性问题会被跳过，并写入 `promoted_at` 表示已处理，不会再次参与晋升筛选。

无论是否存在晋升候选，任务都会执行过期清理：最多处理 500 条 `expires_at <= now` 的短期记忆，删除其 ChromaDB 向量和数据库元数据；该步骤不调用模型。
