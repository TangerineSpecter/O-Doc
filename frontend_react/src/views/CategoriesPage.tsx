...
export default function CategoriesPage() {
    ...
    return (
        <div
            className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24"
            ...
            <ConfirmationModal
                ...
                title="确认删除分类?"
                description={<span>确定要删除该分类吗？删除后，该分类下的文章将自动归入 <span
                    className="font-bold text-slate-800">未分类</span> 状态。</span>}
                confirmText="确认删除"
                type="danger"
            />
            ...
            {/* Category Strip */}
            <div
                className="sticky top-[64px] z-30 bg-[#F9FAFB]/95 backdrop-blur-md -mx-4 px-4 sm:mx-0 sm:px-0 py-2 mb-6 border-b border-slate-200/50 transition-all">
                        ...
                        return (
                            <button key={cat.categoryId} onClick={() => setSelectedCatId(cat.categoryId)}
                                ...
                                <div className="flex flex-col items-start"><span
                                    className={`text-xs font-bold whitespace-nowrap ${isSelected ? 'text-slate-900' : 'text-slate-700'}`}>{cat.name}</span>
                                </div>
                                {isSelected && <span
                                    className="ml-1 text-[10px] bg-orange-100 text-orange-600 px-1.5 rounded-full font-medium">{cat.articleCount}</span>}
                            </button>
                        );
                    })}
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-4">
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            <div
                                className={`p-1.5 rounded-lg ${getThemeStyles(activeCategory.themeId).bg} ${getThemeStyles(activeCategory.themeId).text}`}>{getCategoryIcon(activeCategory)}</div>
                    ...
                    <div className="flex items-center gap-3 self-end">
                        <span
                            className="text-xs text-slate-400 font-medium hidden sm:inline-block">共 {displayArticles.length} 篇</span>
