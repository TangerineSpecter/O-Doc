...
import StoryTimeline from '../components/BookAnalysis/StoryTimeline';
...
export default function BookAnalysisPage() {
    ...
    useEffect(() => {setView(null); selectNode(''); setNodeHistory({scope: '', ids: []}); setCenter(''); setKind(''); setGraphPage(1);}, [status?.mode]);
    ...
    const chooseChapter = (id: string) => {setChapterId(id); setCenter(''); setNodeHistory({scope: '', ids: []}); selectNode('');};
    return <main className="min-h-[calc(100vh-4rem)] bg-slate-50 px-3 py-5 sm:px-5 lg:px-7">
        ...
        <div className="mx-auto mt-4 flex max-w-[1680px] flex-col items-start gap-4 lg:flex-row">
            <aside className="w-full shrink-0 rounded-xl border border-slate-200 bg-white p-3 lg:sticky lg:top-20 lg:w-56">
                <div className="mb-3 rounded-lg bg-slate-50 p-2.5">
                    <div>
                        ...
                        <Select
                            ...
                            onChange={value => setThroughChapter(value ? Number(value) : undefined)}
                ...
                </div><div className="mb-2 flex items-center justify-between px-2 text-xs font-bold text-slate-600"><span>章节与范围</span><span className="text-[10px] font-normal text-slate-400">{chapters.total} 章</span></div><button onClick={() => chooseChapter('')} className={`mb-1 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs ${!chapterId ? 'bg-orange-50 text-orange-700' : 'text-slate-500 hover:bg-slate-50'}`}><Layers className="h-3.5 w-3.5"/>全部已分析内容</button>
            ...
            <section className="w-full min-w-0 flex-1 space-y-4 lg:w-auto">
                ...
                {guide && status.canManage && <ChapterBoundary key={guide.id} bookId={bookId} chapter={guide} disabled={running} onSaved={() => {chooseChapter(''); setThroughChapter(undefined); onSaved();}}/>}
                ...
                {view === 'overview' ? <article className="rounded-xl border border-slate-200 bg-white p-6"><p className="text-[10px] text-orange-600">BOOK OVERVIEW</p><h2 className="mt-2 font-serif text-xl font-bold text-slate-800">{status.overview.complete ? '全书总结' : '已分析章节概要'}</h2><p className="mt-2 text-[11px] text-slate-400">覆盖 {status.overview.coveredChapters?.length || 0} / {status.overview.totalChapters || status.inspection.chapterCount || 0} 章</p><p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-600">{status.overview.summary || '章节完成后会逐步整理概要。'}</p></article> : view === 'digest' ? <>{contentLoading && <p className="p-4 text-xs text-slate-400">加载章节导读…</p>}{guideError && <p role="alert" className="text-xs text-red-600">{guideError}</p>}{guide?.digest ? <GuideDigest title={guide.title} digest={guide.digest} nodes={graph.nodes} onSelect={onSelect} onRead={onRead}/> : <div className="rounded-xl border border-slate-200 bg-white p-8 text-center text-sm text-slate-400">{chapterId ? '该章尚未完成分析。' : '选择左侧章节，查看总结、重点和问答。'}</div>}</> : <>
                    ...
                    {graph.nodes.length > 0 && view === 'timeline' && <StoryTimeline key={`${status.revisionId}:${chapterId}:${thread}:${order}:${graphPage}`} graph={graph} order={order} selectedId={selectedNode} page={graphPage} onPage={setGraphPage} onSelect={onSelect} onRead={onRead}/>}
            ...
            {selectedNode && status.revisionId && <ReadingNodePanel bookId={bookId} revisionId={status.revisionId} nodeId={selectedNode} graph={graph} canManage={status.canManage} refreshKey={refreshKey} throughChapter={throughChapter} canGoBack={previousNodes.length > 0} onSelect={onRelatedSelect} onBack={onNodeBack} onClose={() => {setNodeHistory({scope: '', ids: []}); selectNode('');}} onRead={onRead} onSaved={onSaved}/>}
        </div>
    </main>;
}
