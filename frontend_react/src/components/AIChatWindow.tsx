...
import { X, Minimize2, Send, Bot, BookOpen, Trash2, User } from 'lucide-react';
...
export const AIChatWindow = ({ isOpen, onClose }: AIChatWindowProps) => {
    ...
    const [useKb, setUseKb] = useState(false); // 是否开启知识库
    ...
    if (isMinimized) {
        return (
            ...
            >
                ...
                <span className="text-[10px] font-bold writing-vertical-rl tracking-widest opacity-80 group-hover:opacity-100">AI对话</span>
            </div>
        );
    }
    ...
    // --- 正常窗口状态 ---
    return (
        <div className="fixed right-6 bottom-6 w-[380px] h-[600px] bg-white rounded-2xl shadow-2xl border border-slate-200 z-[100] flex flex-col animate-in slide-in-from-bottom-10 duration-300 ring-1 ring-slate-900/5">
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50/80 backdrop-blur-sm rounded-t-2xl">
            ...
            <div className="p-4 bg-white border-t border-slate-100 rounded-b-2xl">
                ...
                {/* Input Area - 修改部分：使用 Flex 布局解决重叠问题 */}
                <div className="flex items-center gap-2 p-1.5 bg-slate-50 border border-slate-200 rounded-xl focus-within:ring-2 focus-within:ring-orange-500/20 focus-within:border-orange-500 transition-all">
                    <textarea
                        value={input}
                        ...
                        // 移除 absolute 相关的样式，改为 flex-1 撑满剩余空间，bg-transparent 继承容器背景
                    ...
                    <button 
                        ...
                        // 移除 absolute，使用 flex 布局的 shrink-0 防止被压缩
