...
export default function ClassicFloatingMenu() {
      ...
      `}</style>
            ...
            <svg className="absolute inset-0 overflow-visible pointer-events-none" style={{zIndex: 0}}>
                {menuItems.map((_, index) => {
                    ...
                    return (
                        <line key={`line-${index}`} x1={0} y1={0} x2={pos.x} y2={pos.y}
                              stroke={isOpen ? "#cbd5e1" : "transparent"} strokeWidth="2" strokeDasharray="6 6"
                              strokeLinecap="round" className="transition-all duration-500 ease-out"
                              style={{opacity: isOpen ? 0.3 : 0, transitionDelay: isOpen ? '50ms' : '0ms'}}/>
                    )
                })}
            </svg>
                    ...
                    return (
                        <div key={item.id}
                            ...
                            <div className="relative group flex flex-col items-center">
                                ...
                                <button aria-label={item.label} tabIndex={isOpen ? 0 : -1} onClick={() => handleItemClick(item)}
                                        className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg hover:scale-110 active:scale-95 transition-all duration-300 border-[3px] border-white ${item.color} ${item.shadow}`}>{item.icon}</button>
                            </div>
                        </div>
                    );
                })}
            ...
            {/* --- 主按钮 --- */}
            <button
                aria-label={isOpen ? "收起快捷菜单" : "打开快捷菜单"}
        ...
        `}
            ...
            >
                ...
                {!isOpen && starAnimState === 'idle' && (
                    <>
                        <span
                            className="absolute -inset-1 rounded-full border border-orange-400/40 animate-ping opacity-75"
                            style={{animationDuration: '2s'}}></span>
                        <span
                            className="absolute -inset-1 rounded-full border border-orange-300/20 animate-ping opacity-75"
                            style={{animationDuration: '2s', animationDelay: '0.6s'}}></span>
                    </>
                )}
          ...
          `}
                ...
                >
                    {!isOpen && (
                        <>
                            <div
                                className="absolute top-0 left-1/4 w-1/2 h-3/4 bg-gradient-to-b from-white/40 to-transparent rounded-full blur-[2px]"></div>
                            <div
                                className="absolute bottom-0 w-full h-1/3 bg-gradient-to-t from-orange-300/30 to-transparent mix-blend-screen"></div>
                        </>
                    )}
                </div>
                ...
                {/* 叶子 */}
                <div
                    className={`absolute -top-3 -right-2 z-20 transition-all duration-500 origin-bottom-left ${isOpen ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`}
                    style={{animation: !isOpen ? 'leaf-sway 3s ease-in-out infinite alternate' : 'none'}}>
                ...
                <div className="relative z-30 w-full h-full flex items-center justify-center pointer-events-none">
                    {/* Menu Icon */}
                    <svg
                        className={`absolute w-8 h-8 text-slate-600 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isOpen ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-90 scale-50'}`}
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
                        strokeLinejoin="round">
                        <line x1="9" y1="9" x2="9.01" y2="9"/>
                        <line x1="15" y1="9" x2="15.01" y2="9"/>
                        <path d="M7 13s1.5 3 5 3 5-3 5-3"/>
                    </svg>

                    {/* Anime Face */}
                    <div
                        className={`absolute w-full h-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'opacity-0 -rotate-90 scale-50' : 'opacity-100 rotate-0 scale-100'}`}>

                        {/* 状态 1: 痛苦 (>_<) */}
                        {(starAnimState === 'falling' || starAnimState === 'bounce') ? (
                            <svg viewBox="0 0 64 64" className="w-12 h-12">
                                <path d="M12 30 L20 36 L12 42" fill="none" stroke="#451a03" strokeWidth="4"
                                      strokeLinecap="round" strokeLinejoin="round"/>
                                <path d="M52 30 L44 36 L52 42" fill="none" stroke="#451a03" strokeWidth="4"
                                      strokeLinecap="round" strokeLinejoin="round"/>
                                <path d="M24 50 Q 28 46, 32 50 Q 36 54, 40 50" fill="none" stroke="#451a03"
                                      strokeWidth="3" strokeLinecap="round"/>
                                <path d="M56 20 Q 58 24, 54 28" fill="none" stroke="#38bdf8" strokeWidth="3"
                            ...
                            <div className="relative w-full h-full flex items-center justify-center">
                                <svg viewBox="0 0 64 64" className="w-12 h-12">
                                    <path d="M12 34 Q 18 30, 24 34" fill="none" stroke="#451a03" strokeWidth="3.5"
                                          strokeLinecap="round"/>
                                    <g transform="translate(36, 26)">
                                        <ellipse cx="6" cy="8" rx="6" ry="8" fill="#451a03"/>
                                        <circle cx="3.5" cy="4.5" r="2.5" fill="white"/>
                                        <circle cx="7" cy="10" r="1.5" fill="white" opacity="0.4"/>
                                    </g>
                                    <path d="M26 46 Q29 49 32 46 Q35 49 38 46" fill="none" stroke="#451a03"
                                          strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                                    <path d="M8 42 l4 -4 m-2 6 l4 -4" stroke="#f87171" strokeWidth="2" opacity="0.6"/>
                                    <path d="M48 42 l4 -4 m-2 6 l4 -4" stroke="#f87171" strokeWidth="2" opacity="0.6"/>
                                </svg>
                                <svg viewBox="0 0 24 24"
                                     className="absolute top-2 left-1 w-5 h-5 text-yellow-300 drop-shadow-sm"
                                     style={{animation: 'pop-star 0.6s ease-out forwards'}}>
                            ...
                            <svg viewBox="0 0 64 64" className="w-12 h-12">
                                ...
                                <path d="M26 46 Q29 49 32 46 Q35 49 38 46" fill="none" stroke="#451a03"
                                      strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                                <g opacity="0.6" stroke="#f87171" strokeWidth="2" strokeLinecap="round">
                                    <path d="M5 40 L9 36"/>
                                    <path d="M9 40 L13 36"/>
                                    <path d="M51 40 L55 36"/>
                                    <path d="M55 40 L59 36"/>
                                </g>
                            </svg>
                        )}
                    </div>
                ...
                {starAnimState !== 'idle' && (
                    <svg viewBox="0 0 24 24"
                         className="absolute w-6 h-6 text-yellow-400 drop-shadow-md z-40 pointer-events-none" style={{
                        animation:
            ...
            {/* 全屏遮罩 */}
            {isOpen && (
                <div className="fixed inset-0 bg-white/40 backdrop-blur-[3px] z-[-1] animate-in fade-in duration-300"
