...
export default function ImageAnthologySidebar({
  ...
  const hasLocation = locationCityCount > 0;
  const hasFocal = focalLengthTotal > 0;
  const hasTag = tagStats.length > 0 || imageCount > 0;
  const hasColor = baseVisibleImageCount > 0;
  const hasAnySection = hasLocation || hasFocal || hasTag || hasColor;
  ...
  return (
    <aside className="self-start rounded-xl border border-slate-200 bg-white/85 p-4 shadow-sm backdrop-blur lg:sticky lg:top-24">
      ...
      {/* 拍摄地点模块（仅在有城市地点数据时展示） */}
      {hasLocation && (
        <div className={hasFocal || hasTag || hasColor ? 'mb-4' : ''}>
          <button
      ...
      {hasFocal && (
        <div>
          {hasLocation && <div className="mb-4 border-t border-slate-100" />}
      ...
      {hasTag && (
        <div>
          {(hasLocation || hasFocal) && <div className="my-5 border-t border-slate-100" />}
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
      ...
      {hasColor && (
        <div>
          {(hasLocation || hasFocal || hasTag) && <div className="my-5 border-t border-slate-100" />}
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
