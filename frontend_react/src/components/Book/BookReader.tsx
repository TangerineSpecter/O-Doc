...
import {quoteSpread} from '../../utils/bookReaderLocation';
...
export default function BookReader({book, onClose, onProgressSaved, initialLocation}: {book: BookItem; onClose: () => void; onProgressSaved: (progress: number) => void; initialLocation?: SourceEvidence}) {
        ...
        const open = async () => {
            try {
                ...
                if (book.format === 'txt') {
                    ...
                    textRestoreProgressRef.current = Math.max(0, Math.min(100, initialLocation?.locator.offset !== undefined ? initialLocation.locator.offset / Math.max(content.length, 1) * 100 : Number(progress.progress) || 0));
                    ...
                    const savedPage = initialLocation?.locator.page ?? Number.parseInt(progress.location, 10);
                    ...
                    const sourceHref = initialLocation?.locator.href;
                    const sourceSection = sourceHref ? epub.spine.spineItems.find((section: {url: string; href: string}) => section.url.replace(/^\//, '') === sourceHref || section.href === sourceHref) : undefined;
                    await Promise.race([rendition.display(sourceSection?.href || sourceHref || progress.location || undefined), new Promise((_, reject) => window.setTimeout(() => reject(new Error('EPUB_RENDER_TIMEOUT')), 30000))]);
    ...
    const quoteLocatedRef = useRef(false);
    useEffect(() => {
        if (!initialLocation?.quote || !txtPages.length || quoteLocatedRef.current || txtPaginating) return;
        const target = quoteSpread(txtPages, initialLocation.quote, txtPagesPerSpread);
        if (target !== null) { quoteLocatedRef.current = true; setCurrentPage(target); }
    }, [initialLocation, txtPages, txtPagesPerSpread, txtPaginating]);
