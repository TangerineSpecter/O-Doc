...
export const useEditor = () => {
    ...
    const parentArticlesRequestRef = useRef(0);
    ...
    useEffect(() => {
        const requestId = ++articleLoadRequestRef.current;
        ...
        loadArticleDetail();
        return () => {
            if (requestId === articleLoadRequestRef.current) {
    ...
    const loadParentArticlesByCollId = async (collId: string) => {
        const requestId = ++parentArticlesRequestRef.current;
        try {
            setLoadingParentArticles(true);
            const articles = await getArticlesByAnthology(collId);
            if (requestId !== parentArticlesRequestRef.current) return;
            // 转换文章列表为父级文章选项格式
            const parentOptions = [
            ...
            if (requestId === parentArticlesRequestRef.current) {
