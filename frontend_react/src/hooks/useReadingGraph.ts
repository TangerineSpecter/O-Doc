...
export function useReadingGraph(bookId: string, revisionId: string, filters: GraphFilters, refreshKey: string) {
    ...
    const {view, chapterId, kind, query, center, order, page, threadGroup, throughChapter, includeInferred} = filters;
    useEffect(() => {
        const controller = new AbortController();
        if (!revisionId) {setGraph({nodes: [], edges: [], total: 0, page: 1, limit: 200}); return;}
        setLoading(true); setError('');
        const timer = window.setTimeout(() => {
            getReadingGraph(bookId, {view, chapterId, kind, query, center, order, page, threadGroup, throughChapter, includeInferred, revisionId}, controller.signal)
    ...
    }, [bookId, revisionId, view, chapterId, kind, query, center, order, page, threadGroup, throughChapter, includeInferred, refreshKey]);
