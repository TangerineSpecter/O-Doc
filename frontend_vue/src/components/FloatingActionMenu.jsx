...
export default function FloatingActionMenu() {
  ...
  const [starAnimState, setStarAnimState] = useState('idle'); // 'idle' | 'throwing' | 'falling' | 'bounce'
  const [showBonk, setShowBonk] = useState(false); // 控制 Bonk 文字显示
  const [eyePos, setEyePos] = useState({ x: 0, y: 0 });
  
  // 解决闭包陷阱
  const stateRef = useRef({ isOpen, starAnimState, isWinking });
  useEffect(() => {
  ...
  // --- 3. 星星动画序列 (含 Bonk!) ---
  const triggerStarSequence = (onComplete) => {
